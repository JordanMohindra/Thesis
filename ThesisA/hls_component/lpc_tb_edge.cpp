...
//  lpc_compress() -> lpc_decompress() and checks that reconstruction is
...
static Result run_case(const EdgeCase& ec)
{
    ...
    lpc_compress(cin, cout_, ec.nSamples, ec.nRamps, ec.nRX);

    long nPkts   = (long)cout_.size();
    long inBits  = (long)ec.nRamps * ec.nSamples * ec.nRX * 32;
    long outBits = nPkts * 256;

    lpc_decompress(cout_, dout, ec.nSamples, ec.nRamps, ec.nRX);
...
{
    std::cout << "==========================================================" << std::endl;
    std::cout << " LPC Edge Case Testbench  (Test 2.4)                     " << std::endl;
